...
class TrendingMovieList extends Component {
  ...
  componentDidMount() {
    fetch(
      "https://api.themoviedb.org/3/trending/movie/week?api_key=0892b357978ad214e2798df9d45b276e"
    )
  ...
  render() {
    return (
      <div className={styles.trendingModuleContainer}>
        <div className={styles.trendingTitle}>Popular Films</div>
        <div className={styles.trendingCardsContainer}>
          {this.state.trends.map(trend => (
            <Link to={"/media/movie/" + trend.id}>
